...
ChatLogic::~ChatLogic()
{
  ...
  //  delete _chatBot;
    ...
    // delete all nodes
    /*No need for delete as long as we use unique pointers*/
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; }); //changed to unique

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            _nodes.emplace_back(std::make_unique<GraphNode>(id)); //added make unique 
    ...
    // identify root node
    GraphNode *rootNode = nullptr; //changed to a unique
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
